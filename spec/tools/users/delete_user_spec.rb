...
require 'spec_helper'
require_relative '../../../lib/tools/users/delete_user'

RSpec.describe RedmineMcpServer::Tools::Users::DeleteUserTool do
  let(:tool) { described_class.new }
  let(:base_url) { RedmineMcpServer.config[:redmine_url] }

  describe '#name' do
    it 'returns the tool name' do
      expect(tool.name).to eq('delete_user')
    end
  end
  ...
  describe '#description' do
    it 'returns a description' do
      ...
      expect(tool.description).to include('Delete a user')
    end
  end
      ...
      it 'deletes the user' do
        ...
        stub_request(:get, "#{base_url}/users/1.json")
          .to_return(status: 200, body: {
            'user' => { 'id' => 1, 'login' => 'jdoe', 'firstname' => 'John', 'lastname' => 'Doe' }
          }.to_json, headers: { 'Content-Type' => 'application/json' })

        stub_request(:delete, "#{base_url}/users/1.json")
        ...
        expect(result[:data][:deleted_user_id]).to eq(1)
        expect(result[:data][:deleted_user_login]).to eq('jdoe')
      end
    end

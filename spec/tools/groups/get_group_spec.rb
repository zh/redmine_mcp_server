...
require 'spec_helper'
require_relative '../../../lib/tools/groups/get_group'

RSpec.describe RedmineMcpServer::Tools::Groups::GetGroupTool do
  let(:tool) { described_class.new }
  let(:base_url) { RedmineMcpServer.config[:redmine_url] }

  describe '#name' do
    it 'returns the tool name' do
      expect(tool.name).to eq('get_group')
    end
  end
  ...
  describe '#call' do
    context 'with valid group id' do
      it 'returns group details' do
        stub_request(:get, "#{base_url}/groups/1.json")
          .to_return(status: 200, body: { 'group' => { 'id' => 1, 'name' => 'Developers' } }.to_json,
    ...
    context 'with include parameter' do
      it 'includes related data' do
        stub_request(:get, "#{base_url}/groups/1.json?include=users%2Cmemberships")
          .to_return(status: 200, body: { 'group' => { 'id' => 1, 'users' => [] } }.to_json,
                     headers: { 'Content-Type' => 'application/json' })

        result = tool.call({ id: 1, include: 'users,memberships' })
